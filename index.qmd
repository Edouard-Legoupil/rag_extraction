---
title: Creating Brief from Evaluation Report
subtitle: An introduction to Retrieval-Augmented Generation with Large Language Model
date: today
...
Artificial Inteligence (AI) is presented as the potential source for the next, actually the  [fifth waves](https://www.nature.com/articles/s41599-019-0253-6)  of the **evidence revolution** (following the 4 previous waves: #1 outcome monitoring, #2 impact evaluation, #3 systematic reviews and #4 knowledge brokering). This actually reflects a situation where after years of evaluation studies and [thousands of published evaluation reports](https://www.uneval.org/evaluation/reports), the new frontier is about **information retrieval and evidence generalization** : How to extract the most relevant findings and recommendations from a context and reuse them in a different but appropriate context?

Retrieval-augmented generation (RAG) is a type of Artificial Inteligence (AI) question-answering framework that [surfaced in 2020](https://arxiv.org/pdf/2005.11401) and that synergizes the capabilities of Large Language Model (LLMs) and information retrieval systems, aka text mining. Indeed, in some situations, relying on automated retrieval can ensure that the summarization is "*objective*" and do not omit key elements... This paper is presenting the challenges and opportunities associated with those new approaches. It target a technical audience that would like to include such technique within their personal information management toolset, and this using open souce tool working safely fully offline.

First, we explain how to create a [two-pager evaluation brief](https://github.com/Edouard-Legoupil/rag_extraction/raw/main/generated/Evaluation_Brief_response_text_bert.docx) using an orchestration of models and functions. Rather than relying on ad-hoc user interactions through a black box chat interface, it is possible to set up a documented and reproducible summarization scripts that can directly output a word document. The same approach could be applied to other textual analysis, for instance: extract causal chains from the transcriptions of Focus Group Discussions, generate potential theories of change from needs assessment reports or review the usage of Evidence within Country Strategic Plan...

Second, we review the techniques that can be used to optimize summarisation scripts. We generate a  [second alternative brief](https://github.com/Edouard-Legoupil/rag_extraction/raw/main/generated/Evaluation_Brief_response_mmr_recursivecharactertext_bert.docx), and then create an evaluation dataset to explore the different metrics that can be used to evaluate information retrieval process.
...
A robust RAG system comes with different steps:
...
4)  **Deployment**: Organise AI-ready feedback and data to prepare for fine-tuning.

This paper compile the results of experimentations and findings from other articles and tutorials. RAG should definitely be part of the Information management toolbox. The paper includes both a cookbook with a reproducible recipes so that colleagues can rerun and learn from it. It also contains broader consideration on the usage of AI for public programme evaluations, especailly in the last part of deployment.

The illustration from [HuggingFace](https://huggingface.co/learn/cookbook/en/rag_evaluation) below nicely summarise the architecture of the system

![RAG Evaluation, https://huggingface.co/learn/cookbook/en/rag_evaluation](RAG_workflow.png){fig-align="center"}

We use the the 2019 [Evaluation of UNHCR’s data use and information management approaches](https://www.unhcr.org/sites/default/files/legacy-pdf/5dd4f7d24.pdf) to illustrate the process.

We hope that you can adjust this tutorial to your own use cases! The script below is based on [langChain](https://python.langchain.com/v0.1/docs/use_cases/question_answering/) python library but the same pipeline could also be built with the other main LLM orchestration library called [LlamaIndex](https://docs.llamaindex.ai/en/stable/use_cases/q_and_a/)

## Stage 0: Environment Set up
...
If do not have access to any LLM API, an alternative is to install a local one...

There are again plenty of Open Source LLM options to select from...

::: callout-note
Open-source Large Language Model (LLM) have multiple advantages:
...
There are multiple options to do that - but an easy one is to install [OLLAMA](https://ollama.com/), which offers a wide [variety of open models](https://ollama.com/library) from plenty of providers: [LLama3](https://ollama.com/library/llama3) from Facebook, [gemma](https://ollama.com/library/gemma) from Google, [phi3](https://ollama.com/library/phi3) Microsoft but also [quwen](https://ollama.com/library/qwen) from the chinese AliBaba, [falcon](https://ollama.com/library/falcon) from the Emirati Technology Innovation Institute, or Mixtral from the french Mistral_AI. Langchain as dedicated [module to work with ollama](https://python.langchain.com/docs/integrations/chat/ollama). 

Below, we start with [Mixtral Sparse Mixture-of-Expert](https://mistral.ai/news/mixtral-of-experts/), and specifically the quantized version: [8x7b-instruct-v0.1-q4_K_M](https://huggingface.co/TheBloke/Mixtral-8x7B-Instruct-v0.1-GGUF#explanation-of-quantisation-methods), an open-weight model designed to optimize performance-to-cost ratio, aka small in size to run on a strong laptop but good in performance. This download a file with the model which suze around 26Gb.
...
We can build many different retrievers out of the same vectorstore...
...
Note that from this stage, the following steps may take time to run - this will be highly dependent on the power of your computer - obviously the availability of GPUs - Graphical Processing Unit - will significantly increase the speed! This notebook was built on a thinkpad P53 with a Quadro T1000 GPU.
...
### A second report with different embedding, retriever and LLM!

Let's try to generate a second report with different setttings...

Let's try with a second embedding model...  [Hugging face](https://huggingface.co/models?pipeline_tag=feature-extraction&sort=trending) has many options... and there is even a [leaderboard](https://huggingface.co/spaces/mteb/leaderboard) to see how they compete... We will select here the emebeding model from the Beijing Academy of Artificial Intelligence:[bge-large-en-v1.5](https://huggingface.co/BAAI/bge-large-en-v1.5). It is small in size but performant and it does not comsume too much memory. 
...
```
 ...
 ...create our retriever now with mmr...
...
When reading the two reports, we can see significant difference... which is ok as it would be if it was human doing it.. Though, it remains key to assess the quality of the summarization! 
...
We can use a [dedicated model](https://docs.ragas.io/en/stable/howtos/customisations/ragas_custom_model.html#) to critise the first one
...
As presented in [Gartner AI readiness framework](https://www.gartner.com/en/information-technology/topics/ai-readiness), there are potential graduated steps in terms of deployment.

Building common knowledge first on "data retrieval" could be first achievable target. Given the size of linked investment, pooling expertise (sharing scripts) and capacity (server infrastructure) around this objective could make sense. 
