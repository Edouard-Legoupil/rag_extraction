...
Make sure to install the last version of python and create a dedicated python environment to have a fresh install where to manage correctly all the dependencies between packages. This can be done with [conda](https://docs.conda.io/en/latest/){target="_blank"} python modules management utility.
...
Here we start with testing with [GPT4ALL embedding](https://docs.gpt4all.io/gpt4all_python_embedding.html){target="_blank"}
...
Let's try first with a second embedding model... [Hugging face](https://huggingface.co/models?pipeline_tag=feature-extraction&sort=trending){target="_blank"} has many options... and there is even a [leaderboard](https://huggingface.co/spaces/mteb/leaderboard){target="_blank"} to see how they compete... We will select here the embedding model [bge-large-en-v1.5](https://huggingface.co/BAAI/bge-large-en-v1.5){target="_blank"} from the Beijing Academy of Artificial Intelligence. It is small in size but efficient and it does not consume too much memory.
...
dataset_command_parent_recursivecharactertext_bge_d.to_excel("generated/dataset_command_parent_recursivecharactertext_bge.xlsx") 
```
...
dataset_command_ensemble_recursivecharactertext_bge_d.to_excel("generated/dataset_command_ensemble_recursivecharactertext_bge.xlsx") 
```
...
-   [**Faithfulness**](https://docs.ragas.io/en/stable/concepts/metrics/faithfulness.html){target="_blank"} (also called Cgroundedness): Measures whether the LLM outputs are based on the provided ground truth.
...
RAGAS require another LLM to do the assessment. We can use a [dedicated model](https://docs.ragas.io/en/stable/howtos/customisations/ragas_custom_model.html#){target="_blank"} as a critic of the first one.

Let's

```{python}
 # use [Huggingface Endpoints](https://python.langchain.com/v0.1/docs/integrations/llms/huggingface_endpoint/){target="_blank"} for that! The Hugging Face Hub is a platform with over 120k models, 20k datasets, and 50k demo apps (Spaces), all open source and publicly available, in an online platform where people can easily collaborate.
# import os
...
[![From Gartner: https://www.gartner.com/en/information-technology/topics/ai-readiness](img/GL0vYd_WcAAqUDZ.png){fig-align="center"}](https://www.gartner.com/en/information-technology/topics/ai-readiness){target="_blank"}
...
An interesting blog related to [AI costing](https://www.linkedin.com/pulse/decoding-true-cost-generative-ai-your-enterprise-maryam-ashoori-phd/){target="_blank"}
